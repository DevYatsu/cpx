...
pub fn fast_copy(
    ...
    {
        let _ = (source, destination, file_size, overall_pb, options);
        Ok(false)
    }

    #[cfg(target_os = "macos")]
    {
        // On macOS, try to use reflink (clonefile) as a "fast" path if allowed.
        // This is extremely efficient on APFS.
        if options.reflink != Some(ReflinkMode::Never) {
            // Check if we need to remove destination first (matching Linux behavior)
            ...
            match reflink_copy::reflink(source, destination) {
                ...
                Err(_) if options.reflink == Some(ReflinkMode::Always) => {
                ...
                Err(_) => {
                    // Fallback to standard copy if reflink fails and not required to be always
                    return Ok(false);
                }
            }
        }
